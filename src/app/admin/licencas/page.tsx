...
import {
  ...
  ativarLicenca, revogarLicenca, labelPlano, corPlano,
  type Licenca, type PlanoLicenca
} from '@/lib/licencas'

const PLANOS: { value: PlanoLicenca; label: string; desc: string; dias: number }[] = [
  { value: 'basico',       label: 'Básico',       desc: 'Até 50 alunos · 1 ano',    dias: 365 },
  { value: 'profissional', label: 'Profissional', desc: 'Ilimitado · 1 ano',         dias: 365 },
  { value: 'enterprise',  label: 'Enterprise',   desc: 'Multi-escola · 2 anos',     dias: 730 },
]
...
export default function LicencasPage() {
  ...
  const [plano, setPlano]             = useState<PlanoLicenca>('profissional')
  ...
  const [tab, setTab]                 = useState<'gerar' | 'ativar' | 'lista'>('ativar')
  ...
  const gerar = (e: React.FormEvent) => {
    ...
    const lic = criarLicenca(plano, escola.trim())
    salvarLicenca(lic)
    setNovaLic(lic)
    setLicencas(getLicencasGeradas())
    setEscola('')
  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: 860, margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: '0.5rem' }}>🔑 Licenças</h1>
      <p style={{ color: '#6b7280', marginBottom: '1.5rem', fontSize: '0.9rem' }}>Gerencie as licenças de acesso ao sistema</p>

      {/* Licença ativa */}
      <div style={{ background: ativa ? '#f0fdf4' : '#fef9c3', border: `1px solid ${ativa ? '#86efac' : '#fde047'}`, borderRadius: 12, padding: '1.25rem 1.5rem', marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem' }}>
        <div>
          ...
          {ativa ? (
            <div style={{ fontSize: '0.85rem', color: '#374151' }}>
              <strong>{ativa.chave}</strong> · {labelPlano(ativa.plano)} · <strong>{ativa.escola}</strong><br />
              Válida até <strong>{fmtData(ativa.validaAte)}</strong> ({diasRestantes(ativa.validaAte)} dias restantes)
            </div>
          ) : (
            <div style={{ fontSize: '0.85rem', color: '#713f12' }}>Ative uma licença para liberar todos os recursos</div>
          )}
        </div>
        {ativa && (
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem' }}>
        ...
        <button style={tabStyle('gerar')}  onClick={() => setTab('gerar')}>Gerar Nova Chave</button>
        <button style={tabStyle('lista')}  onClick={() => setTab('lista')}>Histórico ({licencas.length})</button>
      </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '1.5rem' }}>
          ...
          <form onSubmit={ativar} style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            <input
              ...
              placeholder="EDU-XXXX-XXXX-XXXX-XXXX"
              required
              style={{ flex: 1, minWidth: 240, padding: '0.65rem 1rem', borderRadius: 8, border: '2px solid #e5e7eb', fontSize: '1rem', fontFamily: 'monospace', letterSpacing: '0.05em' }}
            />
            <button type="submit"
          ...
          <p style={{ color: '#9ca3af', fontSize: '0.8rem', marginTop: '0.75rem' }}>
            Formato: EDU-XXXX-XXXX-XXXX-XXXX (Básico) · PRO-XXXX-XXXX-XXXX-XXXX (Profissional) · ENT-XXXX-XXXX-XXXX-XXXX (Enterprise)
          </p>
        </div>
      )}
      ...
      {tab === 'gerar' && (
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '1.5rem' }}>
          <h2 style={{ fontWeight: 600, fontSize: '1rem', marginBottom: '1.25rem' }}>Gerar Nova Chave de Licença</h2>

          <form onSubmit={gerar}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
              <div>
                <label style={{ display: 'block', fontWeight: 600, fontSize: '0.875rem', marginBottom: '0.4rem' }}>Nome da Escola *</label>
              ...
              <div>
                <label style={{ display: 'block', fontWeight: 600, fontSize: '0.875rem', marginBottom: '0.4rem' }}>Plano *</label>
                <select value={plano} onChange={e => setPlano(e.target.value as PlanoLicenca)}
                  style={{ width: '100%', padding: '0.65rem 1rem', borderRadius: 8, border: '1px solid #d1d5db', fontSize: '0.95rem', background: '#fff' }}>
                  {PLANOS.map(p => <option key={p.value} value={p.value}>{p.label} — {p.desc}</option>)}
                </select>
              </div>
            </div>
            <button type="submit"
          ...
          {novaLic && (
            <div style={{ marginTop: '1.5rem', background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 10, padding: '1.25rem' }}>
              <div style={{ fontWeight: 700, color: '#166534', marginBottom: '0.75rem' }}>✅ Chave gerada!</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
                <code style={{ background: '#fff', border: '1px solid #d1d5db', padding: '0.6rem 1rem', borderRadius: 8, fontSize: '1.1rem', fontWeight: 700, letterSpacing: '0.05em', color: '#1e40af' }}>
                  {novaLic.chave}
                </code>
              ...
              <div style={{ marginTop: '0.75rem', fontSize: '0.85rem', color: '#374151' }}>
                <strong>Escola:</strong> {novaLic.escola} · <strong>Plano:</strong> {labelPlano(novaLic.plano)} · <strong>Válida até:</strong> {fmtData(novaLic.validaAte)}
              </div>
            </div>
          )}
        </div>
      ...
      {tab === 'lista' && (
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, overflow: 'hidden' }}>
          {licencas.length === 0 ? (
            <p style={{ padding: '2rem', textAlign: 'center', color: '#9ca3af' }}>Nenhuma licença gerada ainda.</p>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {['Chave', 'Escola', 'Plano', 'Válida até', 'Status', ''].map(h => (
                    ...
                    <tr key={l.chave} style={{ borderTop: '1px solid #f0f0f0', background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                      <td style={{ padding: '0.75rem 1rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <code style={{ fontSize: '0.8rem', fontWeight: 700, color: '#374151' }}>{l.chave}</code>
                      ...
                      <td style={{ padding: '0.75rem 1rem', fontSize: '0.875rem', color: expirada ? '#dc2626' : '#374151' }}>
                        {fmtData(l.validaAte)}{expirada && ' (expirada)'}
                      </td>
                      <td style={{ padding: '0.75rem 1rem' }}>
                        {l.ativa
